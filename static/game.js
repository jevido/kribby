...
// Don't import every image ever frame fucktard
let images = {
  lion: new Image(),
  duck: {
    right: new Image(),
    left: new Image(),
    fallingRight: new Image(),
    fallingLeft: new Image(),
  },
  flip: {
    lion: new Image(),
    duck: new Image(),
...
images.lion.src = "mobs/Lion/Lion_1.png";
images.duck.right.src = "mobs/Duck/Duck_1.png";
images.duck.left.src = "mobs/Duck/Duck_1_flip.png";
images.duck.fallingLeft.src = "mobs/Duck/Duck_9_flip.png";
images.duck.fallingRight.src = "mobs/Duck/Duck_9.png";
images.flip.lion.src = "mobs/Lion/Lion_1_flip.png";
images.flip.duck.src = "mobs/Duck/Duck_1_flip.png";
...
function collectTreasure(t) {
  ...
  if (player.collected >= treasure.length) {
    ...
    if (getCookie("death") <= 10) playSound("victory"); //no death victory earn a prize maybe?
    if (getCookie("level") >= 4) {
      alert(
        "Nice!, you got trough. As a true duckling you have died only a mere " +
          getCookie("death") +
          " times",
      );
...
function updateEntity(entity, dt) {
  ...
  //normal gravity
  if (entity.dy > 0) {
    if ((celldown && !cell) || (celldiagdown && !cellright && nx)) {
      entity.y = t2p(ty);
      entity.dy = 0;
    ...
    if ((cell && !celldown) || (cellright && !celldiagdown && nx)) {
      entity.y = t2p(ty + 1);
      entity.dy = 0;
      cell = celldown;
  ...
  if (entity.dx > 0) {
    if ((cellright && !cell) || (celldiagdown && !celldown && ny)) {
      entity.x = t2p(tx);
      entity.dx = 0;
    }
  } else if (entity.dx < 0) {
    if ((cell && !cellright) || (celldown && !celldiagdown && ny)) {
      entity.x = t2p(tx + 1);
      entity.dx = 0;
    }
  }
...
function renderPlayer(ctx, dt) {
  ...
  if (player.facing == "left") {
    //going left
    image = player.falling ? images.duck.fallingLeft : images.duck.left;
    ctx.drawImage(
      image,
    ...
    //going right
    image = player.falling ? images.duck.fallingRight : images.duck.right;
    ctx.drawImage(
      image,
      player.x + player.dx * dt,
      player.y + player.dy * dt,
...
function newLevel(levelname) {
  (monsters.length = 0), (treasure.length = 0), (cells.length = 0);
...
if (!checkCookie("death")) setCookie("death", 0, 31);
if (!checkCookie("level")) setCookie("level", 0, 31);
if (getCookie("level") >= 4) {
